...
function onePunch(items){return items && typeof items === 'string' ? items.split(' ').sort().map(val => val.replace(/[ae]/gi, '')).join(' ') : 'Broken!'} //Don't leave this line!!

// Best practice:

// const onePunch = items => (typeof(items) != "string" || items.length == 0) ? "Broken!" : items.split(" ").sort().join(" ").replace(/[ae]/gi, "");

// Clever solution:

// const onePunch = s => s.big && s.length ? s.split(' ').sort().join(' ').replace(/[ae]/ig,'') : "Broken!";
