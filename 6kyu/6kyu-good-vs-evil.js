...
function goodVsEvil(good, evil) {
	...
	const sumGoodPoints = good.split(' ').map((val, ind) => val * goodPoints[ind]).reduce((acc, cur) => acc + cur, 0);
	const sumEvilPoints = evil.split(' ').map((val, ind) => val * evilPoints[ind]).reduce((acc, cur) => acc + cur, 0);
	if (sumGoodPoints > sumEvilPoints) {
		return 'Battle Result: Good triumphs over Evil';
	} else if (sumGoodPoints === sumEvilPoints) {
